...
const Recommendation = () => {
  ...
  const { allocationData, updateAllocationData, calculatePhantomAllocation } =
    useData();
  ...
  const {
    ...
    const shadowSharesAllocation =
      availableForDistribution * (formData.shadowSharesBasePercent / 100);

    const totalUnits = allocationData.salaryGrades.reduce(
      (sum, grade) => sum + grade.units * grade.employees,
      0
    );

    const valuePerUnit =
      totalUnits > 0 ? shadowSharesAllocation / totalUnits : 0;

    const expectedDistributions = allocationData.salaryGrades.map((grade) => {
  ...
  }, [formData, allocationData.salaryGrades]);
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                Phantom Allocation Assistant
              </h1>
              <p className="text-gray-600">
                Configure your shadow share allocation parameters
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("treasuryReserve", e.target.value)
                  }
                ...
                <p className="text-xs text-blue-600 mt-1">
                  Current available treasury funds
                </p>
              </div>
              ...
              <div className="bg-yellow-50 rounded-lg p-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("reserveRatio", e.target.value)
                  }
                ...
                <p className="text-xs text-yellow-600 mt-1">
                  Percentage of profit to keep in reserve ($
              ...
              <div className="bg-purple-50 rounded-lg p-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("shadowSharesBasePercent", e.target.value)
                  }
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between border-t pt-2">
                    <span className="text-gray-600">
                      Shadow Shares ({formData.shadowSharesBasePercent}%):
                    </span>
                    <span className="font-bold text-indigo-600">
                      ${shadowSharesAllocation.toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <h3 className="font-medium text-gray-800 mb-3">
                  Expected Distribution by Grade
                </h3>
                <div className="space-y-2">
                  {expectedDistributions.map((grade) => (
                    <div
                      key={grade.grade}
                    ...
                    >
                      ...
                      <div className="text-right">
                        <div className="font-medium">
                          ${grade.gradePayout.toLocaleString()}
                        </div>
                        <div className="text-xs text-gray-500">
                          {grade.gradeUnits} units × ${valuePerUnit.toFixed(2)}
                        </div>
                      </div>
                    </div>
                  ))}
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mt-6">
          <div className="bg-blue-50 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-blue-900 mb-4">
              Allocation Summary (AI Generated)
            </h3>
            <p className="text-blue-800 leading-relaxed">
              Based on the configured parameters, the shadow share allocation
              has been optimized for fairness and financial sustainability. The{" "}
              {formData.reserveRatio}% reserve ratio ensures adequate treasury
              reserves while the {formData.shadowSharesBasePercent}% allocation
              to shadow shares provides meaningful employee participation in
              company success. The distribution favors higher salary grades
              while maintaining proportional equity across all employee levels.
            </p>
          </div>
        </div>
